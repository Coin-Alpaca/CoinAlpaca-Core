...
/**
 * Fees smaller than this (in uCALP) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 CALP.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-CALP-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    while (true) {
        ...
        CAmount nFeeNeeded = 0; // GetMinimumFee(nBytes, nTxConfirmTarget, mempool);
