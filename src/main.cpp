...
#if defined(NDEBUG)
#error "CoinAlpaca cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uCALP) are considered zero fee (for relaying and mining)
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        // merely non-standard transaction.
        unsigned int nSigOps = GetLegacySigOpCount(tx);
        unsigned int nMaxSigOps = MAX_TX_SIGOPS;
        nSigOps += GetP2SHSigOpCount(tx, view);
...
if (nHeight < Params().LAST_POW_BLOCK())  //First 2 lines are for the pre-mine
        nSubsidy = 10582.02 * COIN;      // block 1 - 1050 = 11,111,110 coins
    else if (nHeight <= 5000)
        nSubsidy = 1 * COIN;
    else if (nHeight > 5000 && nHeight <= 25000) //200,000coins (20,000 block)
        nSubsidy = 10 * COIN;
    else if (nHeight > 25000 && nHeight <= 100000) //1,500,000 coins (75,000 blocks)
        nSubsidy = 20 * COIN;
    else if (nHeight > 100000 && nHeight <= 1050000) //19,000,000 coins (950,000 blocks) 
        nSubsidy = 20 * COIN;
    else if (nHeight > 1050000 && nHeight <= 2100000) //14,700,000 coins (1,050,000 blocks)
        nSubsidy = 14 * COIN;
    else if (nHeight > 2100000 && nHeight <= 3150000) //7,350,000 coins (1,050,000 blocks)
        nSubsidy = 7 * COIN;                       //42,750,000 coin from blocks 5,000-3,150,000 (with pre-mine coins 53,861,110 coins roughly)
    else
        nSubsidy = 2.50 * COIN;
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
	...
	if (!tx.IsCoinBase() && pindexPrev->nHeight >= 1500) {
        if (pvChecks)
        ...
        if (!tx.IsCoinStake()) {
             if (nValueIn < tx.GetValueOut() && pindexPrev->nHeight >= 1500)
            ...
            if (nTxFee < 0 && pindexPrev->nHeight >= 1500)
                return state.DoS(100, error("CheckInputs() : %s nTxFee < 0", tx.GetHash().ToString()),
                    REJECT_INVALID, "bad-txns-fee-negative");
            nFees += nTxFee;
            if (!MoneyRange(nFees) && pindexPrev->nHeight >= 1500)
...
{
    RenameThread("CoinAlpaca-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
   ...
   LogPrint("net", "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->id);
    ...
    if (strCommand == "version") {
        ...
        // CALP: We use certain sporks during IBD, so check to see if they are
